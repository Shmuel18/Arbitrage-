...
class TestHoldOrExit:
    ...
    @pytest.mark.asyncio
    async def test_post_funding_exit_triggers_above_threshold(
        self, controller, config, mock_exchange_mgr, mock_redis
    ):
        """After funding collected, PnL >= post_funding_exit_pct → EXIT early."""
        config.trading_params.exit_offset_seconds = 0
        config.trading_params.profit_target_pct = Decimal("0.7")
        config.trading_params.post_funding_exit_pct = Decimal("0.15")

        # Low funding rates (won't hit full profit target via rate spread)
        past_ms = (time.time() - 1200) * 1000
        for eid in ("exchange_a", "exchange_b"):
            adapter = mock_exchange_mgr.get(eid)
            rate = Decimal("-0.0001") if eid == "exchange_a" else Decimal("0.0001")
        ...
        # Price up 0.3% — above post_funding threshold (0.15%) but below profit_target (0.7%)
        ...
        trade.entry_price_long = Decimal("50000")
        trade.entry_price_short = Decimal("50000")
        # Simulate: funding already collected
        trade._exit_check_active = True
        trade._funding_paid_long = True
        trade._funding_paid_short = True
        trade._funding_paid_at = datetime.now(timezone.utc) - timedelta(minutes=5)
        trade.funding_collections = 1

        await controller._check_exit(trade)

        # Should have exited via post-funding quick exit
        assert trade.trade_id not in controller._active_trades

    @pytest.mark.asyncio
    async def test_post_funding_exit_does_not_trigger_before_funding(
        self, controller, config, mock_exchange_mgr
    ):
        """Before funding is collected, low PnL should NOT trigger post-funding exit."""
        config.trading_params.exit_offset_seconds = 0
        config.trading_params.profit_target_pct = Decimal("0.7")
        config.trading_params.post_funding_exit_pct = Decimal("0.15")

        # Funding not yet paid (next in the future)
        future_ms = (time.time() + 1800) * 1000
        for eid in ("exchange_a", "exchange_b"):
            adapter = mock_exchange_mgr.get(eid)
            rate = Decimal("-0.0001") if eid == "exchange_a" else Decimal("0.0001")
        ...
        # Price up 0.3% — would trigger post-funding exit IF funding was collected
        mock_exchange_mgr.get("exchange_a").get_ticker.return_value = {"last": 50150.0}
        mock_exchange_mgr.get("exchange_b").get_ticker.return_value = {"last": 50000.0}

        trade = _make_trade(controller, spread_pct="1.0", funding_paid=False)
        trade.entry_price_long = Decimal("50000")
        trade.entry_price_short = Decimal("50000")
        # NO funding collections
        trade.funding_collections = 0

        await controller._check_exit(trade)

        # Trade should still be open — no funding collected yet
        assert trade.trade_id in controller._active_trades
        assert trade.state == TradeState.OPEN
