...
class TradingParams(BaseSettings):
    ...
    min_gross_bps: Decimal = Decimal('0.0')
    ...
    enter_before_funding_minutes: int = 15
    exit_after_funding_minutes: int = 15
    min_rate_diff_pct: Decimal = Decimal('0.005')
...
class ExecutionConfig(BaseSettings):
    ...
    concurrent_opportunities: int = 1
...
class Config(BaseSettings):
    ...
    def validate_safety(self):
        ...
        if not self.paper_trading and not self.dry_run:
            ...
            if self.risk_limits.max_margin_usage > Decimal('0.7'):
