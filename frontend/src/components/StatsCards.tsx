...
// Module-level formatter — created once, not per render (avoids Intl overhead).
...
// ── Animated value: counts from 0 on mount, springs on updates ───
    ...
    <div className="nx-stats-layout">
      ...
      <div className="xcards-grid xcards-grid--secondary">
        <StatCard
          ...
          icon={ICON_BAR_CHART}
          accentVar={allTimePnl >= 0 ? '--gold' : '--red'}
          accentHex={allTimePnl >= 0 ? '#d4a843' : '#ef4444'}
        ...
        <StatCard
          ...
          icon={ICON_TARGET}
          accentVar={winRate >= 0.6 ? '--gold' : '--green'}
          accentHex={winRate >= 0.6 ? '#d4a843' : winRate >= 0.4 ? '#f59e0b' : '#ef4444'}
